...
using ImageCtor = void (*)(QImage *, unsigned char *, int, int, qint64,
...
void forward_ctor(const char *name, std::atomic<ImageCtor> &slot, QImage *self,
                  unsigned char *pixels, int width, int height, qint64 stride,
...
// Exact Qt 6/AArch64 constructor entry points. Both are provided because Qt
// builds are allowed to bind either complete-object or base-object aliases.
extern "C" void qimage_external_c1(QImage *self, unsigned char *pixels,
    int width, int height, qint64 stride, QImage::Format format,
    Cleanup cleanup, void *info) asm("_ZN6QImageC1EPhiixNS_6FormatEPFvPvES2_");
extern "C" void qimage_external_c1(QImage *self, unsigned char *pixels,
    int width, int height, qint64 stride, QImage::Format format,
    Cleanup cleanup, void *info) {
    forward_ctor("_ZN6QImageC1EPhiixNS_6FormatEPFvPvES2_", real_c1, self,
                 pixels, width, height, stride, format, cleanup, info);
}

extern "C" void qimage_external_c2(QImage *self, unsigned char *pixels,
    int width, int height, qint64 stride, QImage::Format format,
    Cleanup cleanup, void *info) asm("_ZN6QImageC2EPhiixNS_6FormatEPFvPvES2_");
extern "C" void qimage_external_c2(QImage *self, unsigned char *pixels,
    int width, int height, qint64 stride, QImage::Format format,
    Cleanup cleanup, void *info) {
    forward_ctor("_ZN6QImageC2EPhiixNS_6FormatEPFvPvES2_", real_c2, self,
                 pixels, width, height, stride, format, cleanup, info);
}
